...
impl GitHubProvider {
    ...
    pub async fn collect_insights(
        ...
        // Phase 2: Fetch jobs for each workflow run
        let progress = progress.finish_phase_1_start_phase_2();
        let mut runs_with_jobs = Vec::new();
        for run in workflow_runs {
            runs_with_jobs.push(run);
        }
        info!("Fetched jobs for {} workflow runs", runs_with_jobs.len());

        // Phase 3: Transform to CIInsights
        let progress = progress.finish_phase_2_start_phase_3();
        let insights = CIInsights {
            provider: "GitHub".to_string(),
            project: format!("{}/{}", self.owner, self.repo),
            collected_at: Utc::now(),
            total_pipelines: runs_with_jobs.len(),
            total_pipeline_types: 0,
            pipeline_types: vec![], // TODO: Transform to pipeline types using pipeline_types module
        };
